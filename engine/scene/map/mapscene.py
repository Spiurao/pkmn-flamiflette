...
from engine.graphics.dialogframe import DialogFrame
...
class MapScene(Scene):
    ...
    def showDialog(self, text : str, cb : Callable, payload : Dict):
        if self.__dialogFrame is not None:
            return

        self.lockInputs()
        self.__dialogCallback = cb
        self.__dialogCallbackPayload = payload
        self.__dialogFrame = DialogFrame(self.__window, self.__dialogBoundaries, text, self.onDialogEnd)

    def onDialogEnd(self, choiceResult : int):
        self.__dialogFrame = None
    ...
    def update(self, dt : int, events : List[pygame.event.Event]):
        ...
        # Dialog frame
        if self.__dialogFrame is not None:
            self.__dialogFrame.update(dt, events)
